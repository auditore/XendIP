...
#if DEBUG
    printf("tcpcsum:2 : %d\t%d\n",tcp_hdr->alloc_len,data->alloc_len);
#endif
...
#if DEBUG
    printf("tcp finalize: POS1.1.2 :%c\n",hdrs[strlen(hdrs)-1]);
#endif
			tcpcsum(headers[i],pack,data);
#if DEBUG
    printf("tcp finalize: POS1.1.3 :%c\n",hdrs[strlen(hdrs)-1]);
#endif
		}
