...
int sendpacket(sendip_data *data, char *hostname, int af_type,
							 bool verbose) {
...
int main(int argc, char *const argv[]) {
	...
	while(gnuoptind<argc && (EOF != (optc=gnugetopt(argc,argv,"-x:p:vd:hf:")))) {
		switch(optc) {
		case 'p':
	...
	gnuopterr=1;
	gnuoptind=0;
	while(EOF != (optc=getopt_long_only(argc,argv,"x:p:vd:hf:",opts,&longindex))) {
